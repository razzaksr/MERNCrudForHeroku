...
export const collect=async(id)=>{
    id=id||'';
    const fetch=await axios.get(`${remote}/list/${id}`)
...
export const add=async(object)=>{
    const info=await axios.post(`${remote}/create`,object)
    return info;
}


export const erase=async(id)=>{
    const info=await axios.delete(`${remote}/delete/${id}`)
    return info;
}
...
export const update=async(expert)=>{
    ...
    const rep=await axios.put(`${remote}/update`,expert)
